...
class Player:
    ...
    def move_player1(self, window_height):

        keys = pygame.key.get_pressed()
        if keys[pygame.K_w] and self.positionY >= 0:
            self.positionY -= self.velocity

        if keys[pygame.K_s] and self.positionY <= (window_height - self.height):
            self.positionY += self.velocity

        self.rect_update()

    def move_player2(self, window_height):
